...
def llm_action(obs: dict, task_name: str) -> dict:
    try:
...
What API action should you take? Respond with JSON only."""
        ...
        # Use task-specific fallback
        return FALLBACK_ACTIONS.get(task_name, {"method": "GET", "endpoint": "/users/1", "payload": None})
...
async def run_task(task_name: str):
    ...
    for step_num in range(1, 6):
        try:
            action = llm_action(obs, task_name)
